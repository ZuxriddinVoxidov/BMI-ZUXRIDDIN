...
export default function TeacherReports({
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden pb-4">
      <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">Hisobotlar</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-2xl font-extrabold text-gray-900 dark:text-white">{avgAttendance}%</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">O&apos;rtacha davomat</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
          ...
          <p className="text-2xl font-extrabold text-gray-900 dark:text-white">{topStudent?.name || '-'}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Eng faol o&apos;quvchi</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }}
          ...
          <p className="text-2xl font-extrabold text-gray-900 dark:text-white">{attendanceData.length}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Jami yozuvlar</p>
        </motion.div>
      </div>
      ...
      {monthlyStats.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6">Oylik davomat (%)</h3>
            ...
            <div className="flex items-end gap-3 h-52 min-w-[350px]">
              {monthlyStats.map((m, i) => (
                <div key={i} className="flex flex-col items-center flex-1 min-w-[40px]">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400 mt-2">{m.name}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {studentStats.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">O&apos;quvchilar statistikasi</h3>
            ...
            <table className="w-full min-w-[500px]">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  <th className="text-left py-3 px-4 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Ism</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">To&apos;garak</th>
                  <th className="text-center py-3 px-4 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Keldi</th>
                  <th className="text-center py-3 px-4 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Kelmadi</th>
                  <th className="text-center py-3 px-4 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Sababli</th>
                  <th className="text-center py-3 px-4 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">%</th>
                </tr>
              </thead>
              <tbody>
                {studentStats.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-gray-50 dark:border-gray-800 last:border-0">
                      <td className="py-3 px-4 text-sm font-medium text-gray-900 dark:text-white">{s.name}</td>
                      <td className="py-3 px-4 text-sm text-gray-500 dark:text-gray-400">{s.club}</td>
      ...
      {attendanceData.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Hali davomat ma&apos;lumotlari yo&apos;q</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Davomat olganingizda hisobotlar shu yerda chiqadi</p>
        </div>
      )}
    </div>
  )
