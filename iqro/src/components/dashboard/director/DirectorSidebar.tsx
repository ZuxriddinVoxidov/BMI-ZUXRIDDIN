...
export default function DirectorSidebar({ profile }: Props) {
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="fixed top-4 left-4 z-50 md:hidden p-2 rounded-xl bg-white dark:bg-gray-800 shadow-md border border-gray-100 dark:border-gray-700 touch-manipulation">
        <Menu size={20} className="text-gray-700 dark:text-gray-200" />
      </button>
      {mobileOpen && <div className="fixed inset-0 z-30 bg-black/50 backdrop-blur-sm md:hidden" onClick={() => setMobileOpen(false)} />}
      <aside className={cn('fixed left-0 top-0 h-full bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col z-40 transition-all duration-300', collapsed ? 'w-[70px]' : 'w-[250px]', mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0')}>
        <div className="flex items-center justify-between px-4 h-16 border-b border-gray-100 dark:border-gray-800">
          <Logo href="/director" collapsed={collapsed} subtitle="Direktor paneli" />
          <button onClick={() => setMobileOpen(false)} className="md:hidden p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 touch-manipulation"><X size={18} /></button>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">Direktor</p>
              </div>
            )}
          </div>
        </div>
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                className={cn('flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all touch-manipulation',
                  isActive ? 'bg-amber-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white',
        ...
        <div className="px-3 py-4 border-t border-gray-100 dark:border-gray-800 space-y-1">
          ...
          <button onClick={() => setCollapsed(!collapsed)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 w-full transition-all touch-manipulation">
