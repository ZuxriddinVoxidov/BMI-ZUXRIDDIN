...
export default function DirectorSidebar({ profile }: Props) {
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="fixed top-4 left-4 z-50 md:hidden p-2 rounded-xl bg-white shadow-md border border-gray-100 touch-manipulation">
        <Menu size={20} />
      </button>
      {mobileOpen && <div className="fixed inset-0 z-30 bg-black/50 backdrop-blur-sm md:hidden" onClick={() => setMobileOpen(false)} />}
      <aside className={cn('fixed left-0 top-0 h-full bg-white border-r border-gray-100 flex flex-col z-40 transition-all duration-300', collapsed ? 'w-[70px]' : 'w-[250px]', mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0')}>
        <div className="flex items-center justify-between px-4 h-16 border-b border-gray-100">
          <Logo href="/director" collapsed={collapsed} subtitle="Direktor paneli" />
          <button onClick={() => setMobileOpen(false)} className="md:hidden p-1 rounded-lg hover:bg-gray-100 text-gray-400 touch-manipulation"><X size={18} /></button>
        </div>
        ...
        <div className="px-3 py-4 border-t border-gray-100 space-y-1">
          ...
          <button onClick={() => setCollapsed(!collapsed)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:bg-gray-50 w-full transition-all touch-manipulation">
