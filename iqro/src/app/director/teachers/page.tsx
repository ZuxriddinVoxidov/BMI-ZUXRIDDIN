  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{teachers?.length || 0} ta o&apos;qituvchi</p>
      </div>
              ...
              {t.clubs && t.clubs.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {t.clubs.map((c: { id: string; name: string }) => (
                    <span key={c.id} className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-300">{c.name}</span>
                  ))}
                </div>
              )}
            </div>
