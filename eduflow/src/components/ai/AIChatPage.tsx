...
function exportToPDF(content: string, title: string) {
  ...
  printWindow.document.write(`
    <html>
      ...
      <body>
        <div class="header">
          <h1>EduFlow — 46-maktab</h1>
          <p>${new Date().toLocaleDateString('uz-UZ')}</p>
        </div>
        <div>${content.replace(/\n/g, '<br>')}</div>
        <div class="footer">
          EduFlow tizimi orqali yaratildi • ${new Date().toLocaleString('uz-UZ')}
        </div>
      </body>
    </html>
  `)
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {msg.role === 'assistant' &&
                      ...
                      (userRole === 'teacher' || userRole === 'director') && (
                        <button
                          onClick={() =>
                            exportToPDF(msg.content, `EduFlow Hisobot - ${new Date().toLocaleDateString('uz-UZ')}`)
                          }
