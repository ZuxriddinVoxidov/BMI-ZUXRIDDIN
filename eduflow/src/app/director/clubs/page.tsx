          ...
          <div key={club.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600">{club.category || 'Umumiy'}</span>
              </div>
            </div>
            <div className="space-y-1.5 text-sm text-gray-500">
              <p>👨‍🏫 {club.teacher?.full_name || '—'}</p>
              <p>📅 {club.schedule || '—'}</p>
              ...
              {club.price && club.price > 0 ? (
                ...
                <span className="inline-block text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-medium">Bepul</span>
              )}
            </div>
            <div className="mt-3 pt-3 border-t border-gray-50">
              <span className={`text-xs px-2 py-0.5 rounded-full ${club.is_published ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500'}`}>
